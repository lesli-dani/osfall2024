...
impl MLFQ {
    ...
    pub fn add_process(&mut self, process: Process) {
        ...
        //debug line to check process priority and num levels
        println!("priority = {}, num_levels = {}", process.priority, self.num_levels);

        //check that process priority is less than num_levels
        //process priority must be greater than 0, no need to compare
        //or warning arises "comparison is useless due to type limits"
        if process.priority < self.num_levels {
            self.queues[process.priority].push(process);
        }
        else{
            //if out of range, add with lowest priority, since anything out of range means priority is LOW
            println!("Priority out of range. Process will be added to the end.");
    ...
    pub fn execute_process(&mut self, queue_index: usize) {
        ...
        //access process using queue index
        let mut process = self.queues[queue_index].pop().unwrap();
        
        //updating current_time, if the process's remaining time is bigger
        //it will update current time using time quanta.
        if self.time_quanta[queue_index] < process.remaining_time {
            process.remaining_time -= self.time_quanta[queue_index];
            process.total_executed_time += self.time_quanta[queue_index]; 
            self.current_time += self.time_quanta[queue_index];
        } else {
            process.remaining_time -= process.remaining_time;
            process.total_executed_time += process.remaining_time; 
            self.current_time += process.remaining_time;
        }

        //if the remaining_time is bigger than 0 and it used the entire time_quanta,
        //then move the process to a lower priority queue, if queue index is less than
        //num levels, push to lower queue, else push to the same queue 
    ...
    pub fn priority_boost(&mut self) {
        ...
        let mut idx = 1;
        //while index is less than numlevels
        while idx < self.num_levels {
            // While the queue is not empty pop processes and then boost with priority = 0
            while !self.queues[idx].is_empty() {
                let process = self.queues[idx].pop();
    
                let mut boosted_process = process.unwrap();
                boosted_process.priority = 0;
                self.queues[0].push(boosted_process);
            }
            idx += 1;
        }
